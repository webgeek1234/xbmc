...
{
  CLog::Log(LOGINFO, "App Render checkpoint 1");
  // do not render if we are stopped or in background
  if (m_bStop)
    return;

  CLog::Log(LOGINFO, "App Render checkpoint 2");
  const auto appPlayer = GetComponent<CApplicationPlayer>();
  const auto appPower = GetComponent<CApplicationPowerHandling>();
  ...
  CLog::Log(LOGINFO, "App Render checkpoint 3");
  ...
  CLog::Log(LOGINFO, "App Render checkpoint 4");
  if (!CServiceBroker::GetRenderSystem()->BeginRender())
    return;

  CLog::Log(LOGINFO, "App Render checkpoint 5");
  // render gui layer
  if (appPower->GetRenderGUI() && !m_skipGuiRender)
  {
    CLog::Log(LOGINFO, "App Render checkpoint 6");
    if (CServiceBroker::GetWinSystem()->GetGfxContext().GetStereoMode())
    {
      CLog::Log(LOGINFO, "App Render checkpoint 7");
    ...
    {
      CLog::Log(LOGINFO, "App Render checkpoint 8");
      hasRendered |= CServiceBroker::GetGUI()->GetWindowManager().Render();
    }
    CLog::Log(LOGINFO, "App Render checkpoint 9");
  ...
  CLog::Log(LOGINFO, "App Render checkpoint 10");
  // render video layer
  CServiceBroker::GetGUI()->GetWindowManager().RenderEx();

  CLog::Log(LOGINFO, "App Render checkpoint 11");
  CServiceBroker::GetRenderSystem()->EndRender();

  CLog::Log(LOGINFO, "App Render checkpoint 12");
...
int CApplication::Run()
{
  ...
  while (!m_bStop)
  {
    ...
    CLog::Log(LOGINFO, "App loop checkpoint 1");
    lastFrameTime = std::chrono::steady_clock::now();
    Process();

    CLog::Log(LOGINFO, "App loop checkpoint 2");
    bool renderGUI = GetComponent<CApplicationPowerHandling>()->GetRenderGUI();
    if (!m_bStop)
    {
      CLog::Log(LOGINFO, "App loop checkpoint 3");
      FrameMove(true, renderGUI);
    }

    if (renderGUI && !m_bStop)
    {
      CLog::Log(LOGINFO, "App loop checkpoint 4");
      Render();
    }
    else if (!renderGUI)
    {
      CLog::Log(LOGINFO, "App loop checkpoint 5");
      auto now = std::chrono::steady_clock::now();
    ...
    CLog::Log(LOGINFO, "App loop checkpoint 6");
  }
...
void CApplication::Process()
{
  ...
  CLog::Log(LOGINFO, "Process checkpoint 3");
  {
...
{
  CLog::Log(LOGINFO, "ProcessSlow checkpoint 1");
